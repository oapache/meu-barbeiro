...
import { createLocalAgendamento } from '@/lib/agendamentos'
...
export default function AgendamentoPage({ params }: { params: { id: string } }) {
  ...
  const handleAgendar = async () => {
    ...
    try {
      ...
      // Persistencia interna para manter historico e experiencia 100% no app.
      createLocalAgendamento(payload)

      // Tentativa opcional de envio para API, sem bloquear experiencia local.
      try {
        await ApiService.createAgendamento({
          ...
          data: payload.data,
          hora: payload.hora,
          observacoes: payload.servico_nome,
        })
      } catch {
        // Ignora falha de backend para manter fluxo no app funcional.
      }
